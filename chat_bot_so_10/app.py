...
# ==================================================
LIST_FILES = ['files/dafibik4ff2z', 'files/1ht3zlz36l3q']
STORAGE_FILE = "missing_questions.json"
HISTORY_FILE = "chat_history.json"
ERROR_MESSAGE_TAG = "[MISSING_DOC]"
ERROR_MESSAGE = f"Xin lỗi em, thông tin này hiện chưa có trong thư viện tài liệu của thầy. Thầy sẽ sớm cập nhật kiến thức này nhanh nhất có thể. {ERROR_MESSAGE_TAG} Em có thể hỏi về một chủ đề khác trong chương trình Hóa học/Sinh học THCS không?"
PASSWORD_KEY = "CLEAR_PASSWORD" 
HARDCODED_GREETING = "Xin chào em! Thầy là Gia sư KHTN. Em đang gặp khó khăn ở bài tập hay lý thuyết nào, cứ chia sẻ với Thầy nhé!"
...
# ==================================================
# 🎨 THIẾT KẾ GIAO DIỆN (CUSTOM CSS - LỚP HỌC THÂN THIỆN)
# ==================================================
st.set_page_config(
    page_title="Gia sư KHTN - THCS Phan Chu Trinh", 
    page_icon="👨‍🏫",
    layout="wide"
)
...
<style>
    ...
    /* 4. Tùy biến Bong bóng Chat (Chat Bubbles) */
...
# 📐 QUY TẮC HIỂN THỊ & TRÌNH BÀY
1. KHOẢNG TRẮNG: Sử dụng "Dòng trống" (Double Enter) giữa các đoạn văn.
2. ĐỀ MỤC: Các mục lớn (I, II, III...) phải **IN ĐẬM** và đứng riêng một dòng.
...
4. CÔNG THỨC & LATEX: Công thức bọc trong $...$ hoặc $$...$$.
    ...
    try:
        ...
        # An toàn nạp tài liệu: Bỏ qua file lỗi (tránh crash ứng dụng)
        list_parts = []
        valid_files_count = 0
        for file_id in LIST_FILES:
            try:
                ...
                continue # Nếu file lỗi 403 thì bỏ qua mượt mà
...
# ==================================================
# 📌 SIDEBAR MANAGER (THANH BÊN QUẢN LÝ)
# ==================================================
with st.sidebar:
    st.markdown("### 🏫 Góc Quản Lý")
    
    # Hiển thị trạng thái tài liệu đẹp mắt
    st.markdown(f"""
    <div class="sidebar-card">
        <b style="color: #059669;">📚 Thư viện Khoa học</b><br>
    ...
    if st.session_state.missing_questions:
        st.caption("Các câu hỏi học sinh hỏi nhưng thư viện chưa có:")
        for i, q in enumerate(st.session_state.missing_questions):
            st.markdown(f"**{i+1}.** {q}")
        
        with st.form("clear_form"):
            password = st.text_input("Mật khẩu Giáo viên", type="password")
            if st.form_submit_button("Xóa danh sách", use_container_width=True):
                if password == st.secrets.get(PASSWORD_KEY, "admin123"):
                    ...
                    st.success("✅ Đã dọn dẹp sổ tay!")
                    st.rerun()
                else: st.error("❌ Mật khẩu chưa chính xác.")
    else:
        st.info("Chưa có câu hỏi thiếu trong hệ thống.")

# ==================================================
# 🏛️ GIAO DIỆN CHÍNH (MAIN CLASSROOM DISPLAY)
# ==================================================

# Banner Lớp học Thân thiện
st.markdown("""
<div class="classroom-header">
    <h1>👨‍🔬 GIA SƯ KHTN THCS PHAN CHU TRINH</h1>
    <p>Hỗ trợ học tập Phân môn Hóa học & Sinh học theo Chuẩn GDPT 2018</p>
</div>
""", unsafe_allow_html=True)

# Container chứa đoạn hội thoại
...
prompt = st.chat_input("Em muốn hỏi Thầy điều gì hôm nay...")
...
if prompt:
    ...
    with chat_placeholder:
        ...
        with st.chat_message("assistant", avatar="👨‍🏫"):
            with st.spinner("Thầy đang đọc bài và suy nghĩ..."):
                try:
                    ...
                    st.error(f"Thầy gặp chút sự cố kết nối: {e}")
